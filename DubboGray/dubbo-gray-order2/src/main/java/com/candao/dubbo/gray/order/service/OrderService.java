	...
	public List<Order> getOrders(DistributedContext context) {
		...
		if (context.getGrayBean() != null) {
			...
			if (StringUtils.isEmpty(value)) {
				order.setOrderName("我是order-service2,灰度服务寻找不到,进入正常服务,host:" +RpcContext.getContext().getLocalHost()+":"+RpcContext.getContext().getLocalPort());
			}else{
				order.setOrderName("我是order-service2,灰度策略：" +context.getGrayBean().getGrayKey() + " " + value);
			}
		}else{
			order.setOrderName("我是order-service2,是正常服务,host:" + RpcContext.getContext().getLocalHost()+":"+RpcContext.getContext().getLocalPort());
		}
